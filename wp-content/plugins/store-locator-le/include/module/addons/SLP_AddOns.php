...
class SLP_AddOns extends SLPlus_BaseClass_Object {
	...
	private function get_premier_license_status( $uid = null, $sid = null ) {
		...
		if ( $license_status !== 'valid' ) {
			...
			$last_validation = get_transient( 'slp-premier-last-validation' );
			if ( $last_validation === false ) $last_validation = 0;

			if ( ( time() - $last_validation ) > DAY_IN_SECONDS ) {

				$url = $this->wpdk_url . "license/validate/slp-premier/{$uid}/{$sid}";

				$request = wp_remote_get( $url, array( 'sslverify' => false, 'timeout' => SLPlus::web_timeout, 'http_version' => '1.1' ) );
				...
				set_transient( 'slp-premier-subscription-status', $license_status, defined( 'MYSLP_VERSION' ) ? YEAR_IN_SECONDS : ( DAY_IN_SECONDS * 3 ) );
				set_transient( 'slp-premier-last-validation' , time(), defined( 'MYSLP_VERSION' ) ? YEAR_IN_SECONDS : ( DAY_IN_SECONDS * 3 ) );

			// License was not valid but we just checked it < 1 day ago
			} else {
				$license_status = 'invalid';
				$this->premier_subscription_error = new WP_Error( 'recently_checked_premier_license', __( 'The license was checked less than 24 hours ago.', 'store-locator-le' ) );
			}
		}
