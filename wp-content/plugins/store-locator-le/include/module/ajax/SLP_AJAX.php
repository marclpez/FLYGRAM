...
class SLP_AJAX extends SLP_BaseClass_AJAX {
     ...
     */
    public function add_distance_sort_to_orderby() {
     ...
     * Add sort by distance ASC as default order.
     */
    public function add_initial_distance_sort_to_orderby() {
     ...
     * @return string||null
     */
    public function csl_ajax_onload() {
        return $this->find_locations( 'load' );
    }

    /**
     * Handle AJAX request for Search calls.
     */
    public function csl_ajax_search() {
    ...
    public function find_locations( $mode = 'search' ) {
        ...
        $results = array(
            ...
            'type'         => $mode,
            'http_query'   => array_merge( $this->query_params, array( 'action' => $_REQUEST[ 'action' ] ) ),
            'response'     => $response,
            );
        ...
        return $this->renderJSON_Response( $results );
    }
	 ...
	 * @param $value
	 */
    function update_slp_user_meta( $user_id, $key, $value ) {
        if (!in_array($key,$this->slplus->userMetaKeys)) return;
    ...
    function slp_change_option() {
	    if ( ! check_ajax_referer( 'slp_ajax', 'security', false ) ) {
		    wp_send_json_error( 'Invalid slp_ajax security token sent.' );
		    wp_die();
	    }
        ...
        if ( $plugin_slug === 'user_meta' ) {
	        ...
	        if ( isset( $matches['option_slug'] ) ) {
	            ...
	            if ( $user_id <= 0 ) return;
	            $this->update_slp_user_meta( $user_id , $matches['option_slug'] , $this->formdata['option_value'] );
	        }
	    ...
	    // SLP
            ...
            if ( isset( $matches['option_slug'] ) ) {
	            ...
	            SLP_SmartOptions::get_instance()->execute_change_callbacks();
            }
    ...
    public function slp_delete_location() {
	    ...
	    if ( ! empty( $_REQUEST[ 'screenoptionnonce' ] ) && wp_verify_nonce( $_REQUEST[ 'screenoptionnonce' ] ,'screen-options-nonce'  ) ) {
