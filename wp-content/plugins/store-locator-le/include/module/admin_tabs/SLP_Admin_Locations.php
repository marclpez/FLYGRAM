...
HTML;
		...
		public function display_manage_locations_table() {
			?>
			<div id="manage_locations_modal" title="<?= __( 'Description' , 'store-locator-le' ) ?>"></div>
			<table id='manage_locations_table' class='slplus wp-list-table widefat posts display' cellspacing=0>
				...
				while (
					...
					foreach ( $locations as $location ) {
						...
						$checked = ( ! empty( $_REQUEST['sl_id'] ) && ( in_array( $this->slplus->currentLocation->id, $_REQUEST['sl_id'] ) ) ) ? 'checked' : '';
						?>

						<tr id='location-<?= $this->slplus->currentLocation->id ?>' name='<?= $cleanName ?>' class='slp_managelocations_row <?= $colorClass ?><?= $extraCSSClasses ?>' data-id='<?= $this->slplus->currentLocation->id ?>'
						    data-type='base'>
							<td class='th_checkbox slp_th slp_checkbox'>
								...
								<input type='checkbox' class='slp_checkbox' name='sl_id[]' value='<?= $this->slplus->currentLocation->id ?>' alt='<?= $location_string ?>' title='<?= $location_string ?>' <?= $checked ?>>
								<span class='location_id' data-field='sl_id'><?= $this->slplus->currentLocation->id ?></span>
							</td>
							<td class='actions'>
								<div class='action_buttons'><?= $this->create_action_buttons_html() ?></div>
							</td>
		...
		private function enqueue_scripts() {
			...
			wp_enqueue_style( 'mdi', SLPLUS_PLUGINURL . '/css/materialdesignicons.min.css' );
