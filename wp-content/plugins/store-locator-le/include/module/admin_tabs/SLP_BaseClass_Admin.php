...
if ( ! class_exists( 'SLP_BaseClass_Admin' ) ) {
    ...
    class SLP_BaseClass_Admin extends SLP_Object_With_Objects {
        ...
        public function enqueue_admin_javascript( $hook ) {
	        ...
	        //
			$files = $this->slplus->is_test()
				? array( 'js/admin.js' , 'include/admin.js' )
			    : array( 'js/admin.min.js' , 'js/admin.js' , 'include/admin.min.js' , 'include/admin.js' );
	        ...
	        switch ( $tab ) {
		        ...
		        case 'manage_locations':
			        $files = array( 'js/admin-locations-tab.min.js' , 'js/admin-locations-tab.js' , 'js/' . $base_name . '-admin-locations-tab.min.js', 'js/' . $base_name . '-admin-locations-tab.js' );
			        break;
	        }
		        ...
		        foreach ( $files as $file ) {
			        ...
			        if ( file_exists( $this->addon->dir . $file ) ) {
				        ...
				        if ( ! empty( $tab_js_settings ) ) {
				        	wp_localize_script( $handle, 'wp_data' , $tab_js_settings );
				        }
				        break;
			        }
		        }
