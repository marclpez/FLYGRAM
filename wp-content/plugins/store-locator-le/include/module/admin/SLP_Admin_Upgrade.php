...
class SLP_Admin_Upgrade extends SLPlus_BaseClass_Object {
	...
	private function fix_default_country() {
		if ( version_compare( $this->slplus->installed_version, '4.5.02', '<' ) ) {
			$countries = SLP_Country_Manager::get_instance()->countries;

			$original_country = $this->SmartOptions->default_country->value;
			if ( isset( $countries[ $this->SmartOptions->default_country->value ] ) ) {
				return;
			}
			 ...
			 */
			foreach ( $countries as $new_slug => $country ) {
				$old_slug = sanitize_key( $country->name );
				if ( $old_slug === $original_country ) {
					SLP_SmartOptions::get_instance()->set( 'default_country' , $new_slug );
					break;
				}
			}
		}
