		...
		this.load_app = new Vue({
			...
			methods: {
				...
				load_locations: function() {
				    ...
				    _this.notices = [];
				    _this.notices.push( _this.text.starting_message );

				    async function fetch_location( id ) {
						var response = await fetch( full_url + '/' + id );
                                    ...
                                    .then( function( location_list ) {
									     location_list.forEach(
                                             function( location_data ) {
                                                 fetch_location( location_data.sl_id )
				...
				add_location: function( location ) {
					...
					jQuery.ajax({
						...
						error: function( error_response ) {
							var msg = ( error_response.responseText ) ? error_response.responseText : error_reponse;
							_this.notices = [ msg ];
						}
					});
...
var SLP_Locations_map = function () {
	...
	/**
	 * Get the div to whichever map element we have.
	 *
	 * @param location_element
	 */
	this.get_map_div = function( location_element ) {
		let map_div;

		map_div = location_element.find( '.location_map' );
		if (  map_div.length ) return map_div;

		// New Vue based location map
		map_div = jQuery('#edit_location_map');
		if (  map_div.length ) return map_div;

		// SLP Performance Add On Only
		map_div = location_element.find( '#slp_location_edit_embedmap' );
		if (  map_div.length ) return map_div;
    ...
    this.show_map = function ( location_element ) {
		var center, lat, lng, map_options, marker_options, the_map_div;

        if ( typeof location_element === 'undefined'      ) return;

        the_map_div = this.get_map_div( location_element );
        if (  typeof the_map_div === undefined || ! the_map_div.length      ) return;

        lat =SLP_Location_Manager.Form_Handler.get_value( location_element.find('[data-field="sl_latitude"]') );
        lng =SLP_Location_Manager.Form_Handler.get_value( location_element.find('[data-field="sl_longitude"]') );
        if ( ! lat ) return;
        if ( ! lng ) return;
...
var SLP_Locations_table = function () {
    ...
    this.initialize = function () {
        ...
        jQuery('td[data-field="sl_description"]').click( this.show_description );
	 ...
	 * @param event
	 */
	this.show_description = function (event ) {
		var $my = jQuery( this );
        var data = $my.attr( 'data-value' );
        var modal = jQuery( '#manage_locations_modal' );
        var options = {
        	maxHeight: 400,
        	modal: true,
	        width: 520
...
var SLP_Location_Editor = function () {
    const add_edit_div = jQuery( '#wpcsl-option-add' );
    ...
    this.initialize = function( ) {
	    ...
	    jQuery( '.slp_icon_selector' ).click( function() {
