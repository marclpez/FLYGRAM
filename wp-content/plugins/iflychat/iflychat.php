...
define('IFLYCHAT_PLUGIN_VERSION', 'WP-4.6.4');
...
function iflychat_init()
{
    ...
    if(iflychat_check_access()){
        ...
        wp_enqueue_script('iflychat-ajax', plugin_dir_url( __FILE__ ) . 'js/iflychat.js', array('jquery'), false, true);
...
function iflychat_chatcamp_process_user_data($data) {
    ...
    $data['metadata'] = array("cc_user_roles" => json_encode($data['user_roles']));
    return $data;
}

function iflychat_chatcamp_user_should_update($new, $old) {
    $update = false;
    if($new['user_name'] != $old->display_name){
        $update = true;
    }

    if($new['user_avatar_url'] != $old->avatar_url){
        $update = true;
    }

    if($new['user_profile_url'] != $old->profile_url){
        $update = true;
    }

    if(json_encode($new['user_roles']) !== $old->metadata->cc_user_roles){
        $update = true;
    }
