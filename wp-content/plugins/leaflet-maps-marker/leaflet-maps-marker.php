...
Description: The most comprehensive & user-friendly mapping solution for WordPress (Attention: Leaflet Maps Marker has earned its retirement - existing users get a parting gift: a 50% coupon code for an unexpiring Maps Marker Pro license - for a limited time only, get yours now! Read more & get 50% coupon code at https://www.mapsmarker.com/v3.12.4)
...
Tested up to: 5.6.1
Copyright 2011-2021- MapsMarker.com e.U. - All rights reserved
MapsMarker &reg;
...
if ( is_admin() ) {
	...
	if (is_plugin_active('leaflet-maps-marker-pro/leaflet-maps-marker.php') || (is_plugin_active('maps-marker-pro/maps-marker-pro.php') || class_exists('Maps_Marker_Pro')) ) {
		if (!is_multisite()) {
			exit('[Leaflet Maps Marker - activation failed]: Please deactivate "Maps Marker Pro" first before downgrading to (outdated) "Leaflet Maps Marker" (<a href="https://www.mapsmarker.com/v3.12.4" target="_blank">click here for more details</a>)');
		} else {
			if (is_network_admin()) {
				echo 'Network wide activation of the (outdated) plugin "Leaflet Maps Marker" failed as the plugin "Maps Marker Pro" is still active on subsites. Please activate "Leaflet Maps Marker" on desired subsites only!<br/><a href="https://www.mapsmarker.com/v3.12.4" target="_blank">Click here for more details</a>';
			} else {
				echo 'Too bad you want to use the (outdated) free version again :-( Please deactivate "Maps Marker Pro" first before downgrading to "Leaflet Maps Marker" (<a href="https://www.mapsmarker.com/v3.12.4" target="_blank">click here for more details</a>)!';
			}
		}
	}
}
...
class Leafletmapsmarker
{
	...
	function lmm_update_pointer_footer_script() {
		...
		$pointer_content .= '<p><div style="background:#f99755;display:block;padding:25px;text-decoration:none;color:#2702c6;">' . __('Leaflet Maps Marker has earned its retirement - existing users get a parting gift: a 50% coupon code for an unexpiring<br/>Maps Marker Pro license (<a href="https://demo.mapsmarker.com" target="_blank">view demo</a>).<br/>For a limited time only, get yours now!<p><strong><a href="https://www.mapsmarker.com/v3.12.4" target="_blank">Read more & get 50% coupon code</a></strong>') . '</p></div></p>';
	...
	function lmm_dashboard_widget(){
		...
		echo '<p style="background:#f99755;display:block;padding:5px;text-decoration:none;color:#2702c6;text-align:center;" href="' . LEAFLET_WP_ADMIN_URL . 'admin.php?page=leafletmapsmarker_pro_upgrade">' . __('Leaflet Maps Marker has earned its retirement - existing users get a parting gift: a 50% coupon code for an unexpiring Maps Marker Pro license (<a href="https://demo.mapsmarker.com" target="_blank">view demo</a>).<br/><strong>For a limited time only, <a href="https://www.mapsmarker.com/v3.12.4" target="_blank">get yours now</a>!</strong>','lmm') . '</p><hr style="border:0;height:1px;background-color:#d8d8d8;"/></p>';
		if ($result != NULL) {
	...
	function lmm_install_and_updates() {
		...
		$current_version = "v3124"; //2do - mandatory: change on each update to new version!
	...
	function lmm_plugin_meta_links() {
		...
		function leafletmapsmarker_filter_plugin_meta($links, $file) {
			if ( $file == FB_BASENAME ) {
				array_unshift(
					$links,
					'<a href="https://www.mapsmarker.com/v3.12.4" target="_blank">'.__('Retirement info/coupon code','lmm').'</a>',
