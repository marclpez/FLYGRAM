...
class Tools extends Menu {
	...
	public function query_maps() {
		...
		if (!isset($_POST['nonce']) || wp_verify_nonce($_POST['nonce'], 'mmp-tools-query-maps') === false) {
			wp_send_json(array(
				'results' => array(),
				'more' => false
			));
	...
	public function query_markers() {
		...
		if (!isset($_POST['nonce']) || wp_verify_nonce($_POST['nonce'], 'mmp-tools-query-markers') === false) {
			wp_send_json(array(
				'results' => array(),
				'more' => false
			));
	...
	public function import() {
		...
		if (!$test_mode) {
			if (count($adding)) {
				...
				for ($i = 1; $i <= $markers_batches; $i++) {
					...
					$batch = array_slice($adding, $batch_start, 1000, true);
					...
					if ($result) {
						...
						foreach ($batch as $key => $marker) {
							$details[$key] = array(
								'status' => 4,
								'message' => sprintf(esc_html__('Adding new marker failed', 'mmp'), $marker['id'])
							);
						}
					}
				}
			...
			if (count($updating)) {
				foreach ($updating as $key => $marker) {
					$result = $db->update_marker((object) $marker, $marker['id']);
					if ($result !== false) {
						if ($assignments) {
						...
						$details[$key] = array(
							'status' => 4,
							'message' => sprintf(esc_html__('Updating marker failed', 'mmp'), $marker['id'])
						);
					}
				}
			}
		...
		$summary = sprintf(esc_html__('%1$s markers added, %2$s markers updated, %3$s markers skipped, %4$s errors', 'mmp'), $added, $updated, $skipped, $errors);
					...
					<div class="mmp-batch-settings">
						<form id="mapSettings" method="POST">
							...
							<div id="listContent" class="mmp-map-batch-settings-group">
								...
								<div class="mmp-map-batch-setting">
									<div class="mmp-map-setting-desc">
										<input type="checkbox" id="listGeocodingProviderCheck" class="batch-settings-check" name="listGeocodingProviderCheck" />
										<label for="listGeocodingProvider"><?= esc_html__('Geocoding provider', 'mmp') ?></label>
									</div>
									<div class="mmp-map-setting-input">
										<select id="listGeocodingProvider" name="listGeocodingProvider">
											<optgroup label="<?= esc_attr__('Available providers', 'mmp') ?>">
												<option value="algolia" <?= $settings['listGeocodingProvider'] !== 'algolia' ?: 'selected="selected"' ?>>Algolia Places</option>
												<?php if ($settings['geocodingLocationIqApiKey']): ?>
													<option value="locationiq" <?= $settings['listGeocodingProvider'] !== 'locationiq' ?: 'selected="selected"' ?>>LocationIQ</option>
												<?php endif; ?>
												<?php if ($settings['geocodingMapQuestApiKey']): ?>
													<option value="mapquest" <?= $settings['listGeocodingProvider'] !== 'mapquest' ?: 'selected="selected"' ?>>MapQuest</option>
												<?php endif; ?>
												<?php if ($settings['geocodingGoogleApiKey']): ?>
													<option value="google" <?= $settings['listGeocodingProvider'] !== 'google' ?: 'selected="selected"' ?>>Google</option>
												<?php endif; ?>
												<?php if ($settings['geocodingTomTomApiKey']): ?>
													<option value="tomtom" <?= $settings['listGeocodingProvider'] !== 'tomtom' ?: 'selected="selected"' ?>>TomTom</option>
												<?php endif; ?>
											</optgroup>
