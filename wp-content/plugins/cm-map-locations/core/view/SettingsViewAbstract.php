...
namespace com\cminds\maplocations\view;
...
abstract class SettingsViewAbstract {
	...
	public function renderCategory($category) {
		...
		return '<div class="settings-category settings-category-'. $category .'">'. apply_filters('cmloc-settings-category', $result, $category) .'</div>';
	}
	...
	public function renderSubcategory($category, $subcategory) {
		...
		return '<div class="settings_'. $category .'_'. $subcategory .'">'. apply_filters('cmloc-settings-subcategory', $result, $category, $subcategory) .'</div>';
	}
	...
	public function renderOptionControls($name, array $option = array()) {
		...
		switch ($option['type']) {
			...
			case Settings::TYPE_COLOR:
				return $this->renderInputColor($name);
			...
			case Settings::TYPE_CUSTOM:
				return (is_callable($option['content']) ? call_user_func($option['content'], $name) : $option['content']);
			default:
	...
	protected function renderInputColor($name) {
		return sprintf('<input type="color" name="%s" value="%s" />', esc_attr($name), esc_attr(Settings::getOption($name)));
	}
	...
	protected function renderRichText($name) {
		ob_start();
		wp_editor(Settings::getOption($name), $name . '-richtexteditor', array(
			'textarea_name' => $name,
			'textarea_rows' => 10,
		));
		return ob_get_clean();
	...
	protected function renderMultiCheckboxItem($name, $value, $label, $currentValue = null) {
		...
		if (is_object($currentValue) AND is_callable($currentValue)) {
			$currentValue = call_user_func($currentValue);
		}
