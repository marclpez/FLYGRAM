...
class Labels extends Model {
	...
	const OPTION_LABEL_PREFIX = 'cmloc_label_';
	const TEXT_DOMAIN = 'cm-map-locations';
	...
	public static function initialize() {
		...
		add_action('cmloc_load_label_file', array(__CLASS__, 'loadLabelFile'), 1);
		
		static::loadLabelFile();
		do_action('cmloc_labels_init');
		
		/* You can use the following filters to add new labels for plugin:
		add_filter('cmloc_labels_init_labels', function($labels) {
			$labels['label_name'] = array('default' => 'Value', 'desc' => 'Description', 'category' => 'Other');
			return $labels;
		});
		add_filter('cmloc_labels_init_labels_by_categories', function($labelsByCategories) {
			$labelsByCategories['Other'][] = 'label_name';
			return $labelsByCategories;
		});
		*/
		
		static::$labels = apply_filters('cmloc_labels_init_labels', static::$labels);
		static::$labelsByCategories = apply_filters('cmloc_labels_init_labels_by_categories', static::$labelsByCategories);
