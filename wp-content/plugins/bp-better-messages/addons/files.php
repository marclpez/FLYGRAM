...
if ( !class_exists( 'BP_Better_Messages_Files' ) ):
    ...
    class BP_Better_Messages_Files
    {
	    ...
	    public function remove_attachment($attachment_id){
            ...
            // Get Message
	        $table = $bp_prefix . 'bp_messages_messages';
	        ...
	        if( isset( $message_attachments[$attachment_id] ) ) {
		        $message = $wpdb->get_row( "SELECT * FROM `{$bp_prefix}bp_messages_messages` WHERE `id` = '{$message_id}'" );
        ...
        public function empty_message_fix( &$args, &$errors )
        {
            ...
            if (  !empty( $this->get_unused_attachments( $args[ 'thread_id' ], $args['sender_id'] ) ) ){
                $args[ 'content' ] = ' ';
                unset($errors['empty']);
            }
            ...
            if ( $context !== 'stack' ) {
                if( is_array($attachments) ){
                   if( count( $attachments ) > 0){
                       ...
                       $message .= "<i class=\"fas fa-file\"></i> " . count( $attachments ) . " " . __('attachments', 'bp-better-messages');
                   }
                }
                return $message;
            }
            ...
            if ( !empty( $attachments ) ) {
                ...
                foreach ( $attachments as $attachment_id => $url ) {
                    ...
                    if ( ! $_attachment ) {
                        ...
                        $message = str_replace( array( $url . "\n", "\n" . $url, $url ), '', $message );
                    } else if (strpos( $_attachment->post_mime_type, 'video/mp4') === 0 ) {
        ...
        public function upload_form( $thread_id )
        {
            ...
            <script type="text/javascript">
                ...
                function insertButton() {
                    ...
                    var button   = '<span id="bpbm-upload-btn-<?php echo $thread_id; ?>" class="upload-btn"><i class="fas fa-paperclip" aria-hidden="true"></i><span class="count count-' + count + '">' + count + '</span></span>';
                    <?php } else { ?>
                    var selector = '.bp-messages-wrap.bp-messages-wrap-main[data-thread-id="<?php echo $thread_id; ?>"] .reply form .message, .bp-messages-wrap#bp-better-messages-mini-mobile-container[data-thread="<?php echo $thread_id; ?>"] .reply form .message, .bp-messages-wrap.bp-better-messages-mini .chat[data-thread="<?php echo $thread_id; ?>"] .reply form .message .bp-emojionearea';
                    var button   = '<span id="bpbm-upload-btn-<?php echo $thread_id; ?>" class="upload-btn"><i class="fas fa-paperclip" aria-hidden="true"></i><span class="count count-' + count + '">' + count + '</span></span>';
                    <?php } ?>

                    var selected = $(selector);
                    if( selected.find('#bpbm-upload-btn-<?php echo $thread_id; ?>').length > 0 ) return false;
                    ...
                    if (initiated.length == 0) {
                        ...
                        $('body > .uppy:last-child').addClass('uppy-thread-<?php echo $thread_id ?>');
        ...
        public function handle_delete()
        {
            ...
            // Security verify 1
            if ( ( ! BP_Messages_Thread::check_access( $thread_id, $user_id ) && ! current_user_can('manage_options') ) ||
