...
if ( !class_exists( 'BP_Better_Messages_Ajax' ) ):
    ...
    class BP_Better_Messages_Ajax
    {
        ...
        public function delete_message()
        {
            ...
            foreach( $messages_ids as $message_id ){
                ...
                if( $message->sender_id === $user_id ){
                    ...
                    $sql = $wpdb->prepare("DELETE FROM {$bp_prefix}bp_messages_messages WHERE id = %d", $message->id);
                    $wpdb->query( $sql );
                    $sql = $wpdb->prepare("DELETE FROM {$bp_prefix}bp_messages_meta WHERE message_id = %d", $message->id);
                    $wpdb->query( $sql );
        ...
        public function add_user_to_thread(){
            ...
            if( empty($errors) ) {
                foreach ($users as $username) {
                    ...
                    SELECT COUNT(*) FROM `{$bp_prefix}bp_messages_recipients` WHERE `user_id` = %d AND `thread_id` = %d AND `sender_only` = '0'
                    ", $user->ID, $thread_id));
        ...
        public function get_edit_message(){
            ...
            $message_content = $wpdb->get_var( $wpdb->prepare( "SELECT message FROM {$bp_prefix}bp_messages_messages WHERE id = %d AND sender_id = %d", $message_id, $user_id ) );
        ...
        public function edit_message(){
            ...
            $old_message_content = $wpdb->get_var( $wpdb->prepare( "SELECT message FROM {$bp_prefix}bp_messages_messages WHERE id = %d AND sender_id = %d", $message_id, $user_id ) );
        ...
        public function un_delete_thread()
        {
            ...
            if ( ( BP_Better_Messages()->settings['disableDeleteThreadCheck'] !== '1' && ! wp_verify_nonce( $_POST[ 'nonce' ], 'un_delete_' . $thread_id ) ) || ! $has_access ) {
                ...
                $restored = $wpdb->update( $bp_prefix . 'bp_messages_recipients', array(
                    'is_deleted' => 0
                ), array(
                    'thread_id' => $thread_id,
                    'user_id'   => $user_id
