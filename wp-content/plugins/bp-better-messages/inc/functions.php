    ...
    class BP_Better_Messages_Functions
    {
        ...
        public function is_thread_moderator($user_id, $thread_id){
            ...
            $admin_user = (int) $wpdb->get_var($wpdb->prepare("
                ...
                WHERE `thread_id` = %d 
                AND   `sender_id` != '0'
                ORDER BY `{$bp_prefix}bp_messages_messages`.`date_sent` ASC
                LIMIT 0,1
            ", $thread_id));
        ...
        public function get_threads( $user_id = 0, $exclude_threads = [] )
        {
            ...
            if( is_array( $exclude_threads )  && count( $exclude_threads ) > 0 ){
                ...
                $sql = $wpdb->prepare( "
                    ...
                    FROM
                        {$bp_prefix}bp_messages_recipients as recipients
                    INNER JOIN {$bp_prefix}bp_messages_messages messages 
            ...
            foreach ( $threads as $index => $thread ) {
                $recipients = array();
                $results = $wpdb->get_results( $wpdb->prepare( "SELECT user_id FROM {$bp_prefix}bp_messages_recipients WHERE thread_id = %d", $thread->thread_id ) );
        ...
        public function get_starred_count()
        {
            ...
            return $wpdb->get_var( "
                SELECT
                  COUNT({$bp_prefix}bp_messages_messages.id) AS count
                FROM {$bp_prefix}bp_messages_meta
                  INNER JOIN {$bp_prefix}bp_messages_messages
                    ON {$bp_prefix}bp_messages_meta.message_id = {$bp_prefix}bp_messages_messages.id
                  INNER JOIN {$bp_prefix}bp_messages_recipients
                    ON {$bp_prefix}bp_messages_recipients.thread_id = {$bp_prefix}bp_messages_messages.thread_id
                WHERE {$bp_prefix}bp_messages_meta.meta_key = 'starred_by_user'
                AND {$bp_prefix}bp_messages_meta.meta_value = $user_id
                AND {$bp_prefix}bp_messages_recipients.is_deleted = 0
                AND {$bp_prefix}bp_messages_recipients.user_id = $user_id
            " );
        }
        ...
        public function get_starred_stacks()
        {
            ...
            $query = $wpdb->prepare( "
                SELECT
                  {$bp_prefix}bp_messages_messages.*
                FROM {$bp_prefix}bp_messages_meta
                  INNER JOIN {$bp_prefix}bp_messages_messages
                    ON {$bp_prefix}bp_messages_meta.message_id = {$bp_prefix}bp_messages_messages.id
                  INNER JOIN {$bp_prefix}bp_messages_recipients
                    ON {$bp_prefix}bp_messages_recipients.thread_id = {$bp_prefix}bp_messages_messages.thread_id
                WHERE {$bp_prefix}bp_messages_meta.meta_key = 'starred_by_user'
                AND {$bp_prefix}bp_messages_meta.meta_value = %d
                AND {$bp_prefix}bp_messages_recipients.is_deleted = 0
                AND {$bp_prefix}bp_messages_recipients.user_id = %d
            ", $user_id, $user_id );
        ...
        public function get_search_stacks( $search = '' )
        {
            ...
            $query = $wpdb->prepare( "
                SELECT {$bp_prefix}bp_messages_messages.*
                FROM {$bp_prefix}bp_messages_messages
                INNER JOIN {$bp_prefix}bp_messages_recipients
                ON {$bp_prefix}bp_messages_recipients.thread_id = {$bp_prefix}bp_messages_messages.thread_id
                WHERE
                {$bp_prefix}bp_messages_recipients.is_deleted = 0 
                AND {$bp_prefix}bp_messages_recipients.user_id = %d
                AND {$bp_prefix}bp_messages_messages.message LIKE %s
            ", $user_id, $searchTerm );
        ...
        public function get_avatar($user_id, $size, $args = array()){
            ...
            $r['class'] .= ' bpbm-avatar-user-id-' . $user->ID;

            extract( $r, EXTR_SKIP );

            #print_r($r);
            #$class .= ' bpbm-avatar-user-id-' . $user->ID;
            ...
            $avatar = apply_filters( 'bp_get_member_avatar',
                bp_core_fetch_avatar(
                    array(
                        ...
                        'html'  => $html,
                        'extra_attr' => ' data-user-id="' . $user->ID . '"'
                    )
                ), $r );
        ...
        public function find_existing_threads($from, $to){
            ...
            $query = $wpdb->prepare("SELECT
                ...
                FROM {$bp_prefix}bp_messages_recipients
                  INNER JOIN {$bp_prefix}bp_messages_recipients {$bp_prefix}bp_messages_recipients_1
                    ON {$bp_prefix}bp_messages_recipients.thread_id = {$bp_prefix}bp_messages_recipients_1.thread_id
                WHERE {$bp_prefix}bp_messages_recipients.user_id = %d
                AND {$bp_prefix}bp_messages_recipients.is_deleted = 0
                AND {$bp_prefix}bp_messages_recipients_1.user_id = %d 
                AND {$bp_prefix}bp_messages_recipients.thread_id NOT IN (
                    SELECT meta_value 
                    FROM `{$bp_prefix}postmeta`
                    WHERE post_id IN ( SELECT ID FROM `{$bp_prefix}posts` WHERE `post_type` = 'bpbm-bulk-report')
                    AND meta_key = 'thread_ids'
                    GROUP BY meta_value
                )
                ORDER BY {$bp_prefix}bp_messages_recipients.thread_id ASC
                LIMIT 0, 1", $from, $to);
            ...
            ?><div class="thread <?php echo implode(' ', $classes); ?>"
                ...
                <div class="info">
                    ...
                    <?php if(BP_Better_Messages()->settings['disableSubject'] !== '1' && ! empty( $thread->subject )) { ?>
                        <h4><?php echo wp_unslash($thread->subject); ?></h4>
                    <?php } ?>
                    <p><?php
        ...
        public function get_pm_thread_id( $to, $from = false ){
            ...
            $last_thread = intval($wpdb->get_var("SELECT MAX(thread_id) FROM `{$bp_prefix}bp_messages_messages`;"));
            $thread_id = $last_thread + 1;

            $wpdb->insert(
                "{$bp_prefix}bp_messages_recipients",
                array(
