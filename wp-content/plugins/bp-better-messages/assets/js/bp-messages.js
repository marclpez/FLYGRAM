...
(function ($) {
    ...
    $(document).ready(function () {
        ...
        if( isMobile && BP_Messages['autoFullScreen'] === '1' ) {
            if (__thread || __threads) {
                ...
                var wrap = $('.bp-messages-wrap.mobile-ready.bp-messages-wrap-main');
                openMobile(wrap);
            } else {
        ...
        bpMessagesWrap.on('submit', '.reply > form', function (event) {
            ...
            var participants = $.parseJSON(usersJson);
    ...
    function changeMaxHeight(){
        ...
        jQuery('.bp-messages-wrap.bp-better-messages-list .scroller').css( 'max-height', height - 50 );
        jQuery('.bp-messages-wrap.bp-better-messages-mini .scroller').css( 'max-height', height - 50 );
    }
            ...
            jQuery('.scroller.thread').BPBMoverlayScrollbars({
                ...
                callbacks : {
                    onInitialized: function(){
                        ...
                        if( message_to !== false ){
                            jQuery(this.getElements().host).addClass('user-scrolled');
                            this.scroll({ el : message_to, scroll : "ifneeded", margin : 20 });
                        } else if( jQuery(elements.host).is(':visible') && height === 0 ){
