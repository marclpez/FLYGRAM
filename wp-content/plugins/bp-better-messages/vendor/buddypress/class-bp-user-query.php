	...
	public function prepare_user_ids_query() {
		...
		switch ( $type ) {
			...
			case 'online' :
				...
				$this->uid_table = $bp_prefix . 'bp_activity';
			...
			case 'alphabetical' :
				...
				// the xprofile table.
					...
					$this->uid_name = 'user_id';
					$this->uid_table = $bp_prefix . 'bp_xprofile_data';
					$sql['select']  = "SELECT u.{$this->uid_name} as id FROM {$this->uid_table} u";
		...
		if ( false !== $search_terms ) {
			...
			$sql['where']['search'] = $wpdb->prepare(
				"u.{$this->uid_name} IN ( SELECT ID FROM {$wpdb->users} WHERE ( user_login LIKE %s OR user_login LIKE %s OR user_nicename LIKE %s OR user_nicename LIKE %s OR display_name LIKE %s OR display_name LIKE %s ) )",
