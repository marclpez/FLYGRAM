...
class BP_Messages_Message {
	...
	public function populate( $id ) {
		...
		if ( $message = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$bp_prefix}bp_messages_messages WHERE id = %d", $id ) ) ) {
	...
	public function send() {
		...
		if ( empty( $this->thread_id ) ) {
			$this->thread_id = (int) $wpdb->get_var( "SELECT MAX(thread_id) FROM {$bp_prefix}bp_messages_messages" ) + 1;
			$new_thread      = true;
		}

		// First insert the message into the messages table.
		if ( ! $wpdb->query( $wpdb->prepare( "INSERT INTO {$bp_prefix}bp_messages_messages ( thread_id, sender_id, subject, message, date_sent ) VALUES ( %d, %d, %s, %s, %s )", $this->thread_id, $this->sender_id, $this->subject, $this->message, $this->date_sent ) ) ) {
			return false;
		}
		...
		if ( $new_thread ) {
			...
			foreach ( (array) $this->recipients as $recipient ) {
				$wpdb->query( $wpdb->prepare( "INSERT INTO {$bp_prefix}bp_messages_recipients ( user_id, thread_id, unread_count ) VALUES ( %d, %d, 1 )", $recipient->user_id, $this->thread_id ) );
			...
			if ( ! in_array( $this->sender_id, $recipient_ids ) ) {
				$wpdb->query( $wpdb->prepare( "INSERT INTO {$bp_prefix}bp_messages_recipients ( user_id, thread_id, sender_only ) VALUES ( %d, %d, 1 )", $this->sender_id, $this->thread_id ) );
			}
		} else {
			// Update the unread count for all recipients.
			$wpdb->query( $wpdb->prepare( "UPDATE {$bp_prefix}bp_messages_recipients SET unread_count = unread_count + 1, sender_only = 0, is_deleted = 0 WHERE thread_id = %d AND user_id != %d", $this->thread_id, $this->sender_id ) );
		}
	...
	public function get_recipients() {
		...
		return $wpdb->get_results( $wpdb->prepare( "SELECT user_id FROM {$bp_prefix}bp_messages_recipients WHERE thread_id = %d", $this->thread_id ) );
	}
	...
	public static function get_last_sent_for_user( $thread_id ) {
		...
		$query = $wpdb->get_var( $wpdb->prepare( "SELECT id FROM {$bp_prefix}bp_messages_messages WHERE sender_id = %d AND thread_id = %d ORDER BY date_sent DESC LIMIT 1", bp_loggedin_user_id(), $thread_id ) );
	...
	public static function is_user_sender( $user_id, $message_id ) {
		...
		$query = $wpdb->get_var( $wpdb->prepare( "SELECT id FROM {$bp_prefix}bp_messages_messages WHERE sender_id = %d AND id = %d", $user_id, $message_id ) );
	...
	public static function get_message_sender( $message_id ) {
		...
		$query = $wpdb->get_var( $wpdb->prepare( "SELECT sender_id FROM {$bp_prefix}bp_messages_messages WHERE id = %d", $message_id ) );
