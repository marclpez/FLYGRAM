    ...
    function bp_core_fetch_avatar( $args = '' ) {
            ...
            if( $params['html'] === false ){
                ...
                if( function_exists('get_wp_user_avatar') ){
                    ...
                    return get_avatar($params['item_id'], 50, '', '', $params);
                }
            }
    }
}
...
function bp_messages_delete_meta( $message_id, $meta_key = false, $meta_value = false, $delete_all = false ) {
    ...
    if ( empty( $meta_key ) ) {
        ...
        $keys = $wpdb->get_col( $wpdb->prepare( "SELECT meta_key FROM {$bp_prefix}bp_messages_meta WHERE message_id = %d", $message_id ) );
...
if( ! function_exists('bp_messages_update_meta_cache') ) {
    function bp_messages_update_meta_cache($message_ids = false)
    {
        ...
        bp_update_meta_cache(array(
            ...
            'meta_table' => $bp_prefix . 'bp_messages_meta',
...
function messages_get_message_thread_id( $message_id = 0 ) {
    ...
    return (int) $wpdb->get_var( $wpdb->prepare( "SELECT thread_id FROM {$bp_prefix}bp_messages_messages WHERE id = %d", $message_id ) );
}}
