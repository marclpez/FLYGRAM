...
class BP_Messages_Thread {
    ...
    public function get_recipients( $thread_id = 0 ) {
        ...
        if ( false === $recipients ) {
            ...
            $recipients = array();
            $sql        = $wpdb->prepare( "SELECT * FROM {$bp_prefix}bp_messages_recipients WHERE thread_id = %d", $thread_id );
    ...
    public static function get_messages( $thread_id = 0 ) {
        ...
        if ( false === $messages ) {
            ...
            $messages = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$bp_prefix}bp_messages_messages WHERE thread_id = %d ORDER BY date_sent ASC", $thread_id ) );
    ...
    public static function delete( $thread_id = 0, $user_id = 0 ) {
        ...
        $wpdb->query( $wpdb->prepare( "UPDATE {$bp_prefix}bp_messages_recipients SET is_deleted = 1 WHERE thread_id = %d AND user_id = %d", $thread_id, $user_id ) );

        // Get the message ids in order to pass to the action.
        $message_ids = $wpdb->get_col( $wpdb->prepare( "SELECT id FROM {$bp_prefix}bp_messages_messages WHERE thread_id = %d", $thread_id ) );

        // Check to see if any more recipients remain for this message.
        $recipients = $wpdb->get_results( $wpdb->prepare( "SELECT id FROM {$bp_prefix}bp_messages_recipients WHERE thread_id = %d AND is_deleted = 0", $thread_id ) );
        ...
        if ( empty( $recipients ) ) {
            ...
            $wpdb->query( $wpdb->prepare( "DELETE FROM {$bp_prefix}bp_messages_messages WHERE thread_id = %d", $thread_id ) );
            ...
            $wpdb->query( $wpdb->prepare( "DELETE FROM {$bp_prefix}bp_messages_recipients WHERE thread_id = %d", $thread_id ) );
        }
    ...
    public static function get_current_threads_for_user( $args = array() ) {
        ...
        $sql['from']   = "FROM {$bp_prefix}bp_messages_recipients r INNER JOIN {$bp_prefix}bp_messages_messages m ON m.thread_id = r.thread_id {$meta_query_sql['join']}";
    ...
    public static function get_meta_query_sql( $meta_query = array() ) {
        ...
        if ( ! empty( $meta_query ) ) {
            ...
            // $wpdb->messagemeta.
            $wpdb->messagemeta = $bp_prefix . 'bp_messages_meta';
    ...
    public static function mark_as_read( $thread_id = 0 ) {
        ...
        $bp_prefix = bp_core_get_table_prefix();
        $retval = $wpdb->query( $wpdb->prepare( "UPDATE {$bp_prefix}bp_messages_recipients SET unread_count = 0 WHERE user_id = %d AND thread_id = %d", $user_id, $thread_id ) );
    ...
    public static function mark_as_unread( $thread_id = 0 ) {
        ...
        $bp     = buddypress();
        $bp_prefix = bp_core_get_table_prefix();
        $retval = $wpdb->query( $wpdb->prepare( "UPDATE {$bp_prefix}bp_messages_recipients SET unread_count = 1 WHERE user_id = %d AND thread_id = %d", $user_id, $thread_id ) );
    ...
    public static function get_total_threads_for_user( $user_id, $box = 'inbox', $type = 'all' ) {
        ...
        return (int) $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(thread_id) FROM {$bp_prefix}bp_messages_recipients WHERE user_id = %d AND is_deleted = 0 {$exclude_sender} {$type_sql}", $user_id ) );
    }
    ...
    public static function user_is_sender( $thread_id ) {
        ...
        $sender_ids = $wpdb->get_col( $wpdb->prepare( "SELECT sender_id FROM {$bp_prefix}bp_messages_messages WHERE thread_id = %d", $thread_id ) );
    ...
    public static function get_last_sender( $thread_id ) {
        ...
        if ( ! $sender_id = $wpdb->get_var( $wpdb->prepare( "SELECT sender_id FROM {$bp_prefix}bp_messages_messages WHERE thread_id = %d GROUP BY sender_id ORDER BY date_sent LIMIT 1", $thread_id ) ) ) {
            return false;
        }
    ...
    public static function get_inbox_count( $user_id = 0 ) {
        ...
        if ( false === $unread_count ) {
            ...
            $unread_count = (int) $wpdb->get_var( $wpdb->prepare( "SELECT SUM(unread_count) FROM {$bp_prefix}bp_messages_recipients WHERE user_id = %d AND is_deleted = 0 AND sender_only = 0", $user_id ) );
